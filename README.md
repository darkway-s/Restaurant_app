...
​	这里也体现了对基础要求的一个拓展实现，使得可以对同一时刻到来的相同人数顾客组进行处理。
...
​	陈*偲同学提供了更好的思路，仍然采用优先级队列，但是对于每个人数的组分别安排等位区，即对原队列进行复制扩展：`vector<priority_queue<...> >wait_section;`如此一来，不仅在实现上更容易，在设计上也更舒适。
...
​	整个设计过程经过这样结构化的处理后，就变的非常清晰，思维不会混乱。但是在实施过程需要注意的一点是，**每个**较为简单的问题完成之后都要**好好检验！**否则之后一起debug的过程就不知道到底是哪里出了问题，这样的调试是非常痛苦的，而且很难找。
...
​	开始的时候不小心把id的关系写成了`a.id >= b.id`，运行的时候报了一个assertion failed。

<img src="image-20210109161521285.png" alt="image-20210109161521285" style="zoom: 50%;" />

​	感谢张*元同学的帮助，给了我关于这方面的信息，优先级队列的比较函数必须是符合全序关系，如果用了`>=`就违反了反自反性。

​	如果是改完一大堆内容之后再运行，谁知道是这里发生的问题？甚至无法调试，因为运行就会报错。

​	再比如是resclock类一开始我命名它为clock，但是报错各种未知运算符重载，后来发现是stl库中的clock_t类声明冲突，下次得少用`using namespace std;`。

​	Practice makes Perfect. 犯这些小错误是必经之路。



### 3.及时Git

​	GitHub在项目进行过程中给予了很大帮助，在修改错程序后能够及时地恢复原来可运行版本，并且修改前后的内容比较在调试时十分有用。

​	除此之外，这克服了我贫乏的记忆力，在考试周告一段落时，拿起一周前的事情，仍然能抓住之前的思维轨迹。

![image-20210109162533447](image-20210109162533447.png)

[项目GitHub库]: https://github.com/darkway-s/Restaurant_app/	"饭店PJ"



## 六、参考资料

[1]: https://blog.csdn.net/wenhlin/article/details/78641308	"自顶向下，逐步求精的设计思想"
[2]: https://blog.csdn.net/qq_40008325/article/details/111303671	"全局变量的使用"
[3]: https://blog.csdn.net/u013862444/article/details/80737726	"cin用法详解"
[4]: https://blog.csdn.net/u013354486/article/details/90301282	"error:LNK2005解决办法"
[5]: https://blog.csdn.net/weixin_36888577/article/details/79937886	"priority_queue用法详解"
